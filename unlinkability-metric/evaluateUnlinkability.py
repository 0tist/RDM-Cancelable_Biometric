...
print(len(nonMatedScores), len(matedScores))

if(len(nonMatedScores) > len(matedScores)):
    nonMatedScores = nonMatedScores[:len(matedScores)]
    
elif(len(nonMatedScores) < len(matedScores)):
    matedScores = matedScores[:len(nonMatedScores)]
...
bin_edges = numpy.linspace(min([min(matedScores), min(nonMatedScores)]), max([max(matedScores), max(nonMatedScores)]), num=int(nBins) + 1, endpoint=True)
...
print("************************************************************************")
print("D*y1_2")
#print(numpy.histogram(matedScores, bins = bin_edges, density = True)[1])
